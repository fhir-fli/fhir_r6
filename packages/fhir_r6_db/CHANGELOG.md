...
- The string tests run on Patient here because the generated extractor indexes no string parameter for Observation in R6: ballot 3's `value-string` expression is `value.ofType(string)`, with no resource prefix, and the generator drops it — one of 65 such expressions in that ballot. A generator item, not a database one.
